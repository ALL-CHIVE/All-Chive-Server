...
public class GetCategoryContentsUseCase {
    ...
    public CategoryContentsResponse execute(Long categoryId, Pageable pageable) {
        ...
        Slice<Content> contentList = contentAdaptor.querySliceContentByCategoryId(categoryId, pageable);
        List<ContentTagGroup> contentTagGroupList = contentTagGroupAdaptor
                .queryContentIn(contentList.getContent());
        Slice<ContentResponse> contentResponseSlice = contentList
                .map(content -> contentMapper.toContentResponse(content, contentTagGroupList));
